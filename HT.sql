...
        e.reg_dt_tm,
        e.type_code
    FROM {source_tables["encounter_table"]} e
    INNER JOIN office_visits ov 
        ON ov.encounter_reference = e.encounter_id
    WHERE e.reg_dt_tm BETWEEN ADD_MONTHS(CURRENT_DATE, -12) AND CURRENT_DATE
        AND CAST(e.reg_dt_tm AS DATE) BETWEEN ADD_MONTHS(CURRENT_DATE, -12) AND CURRENT_DATE
...
        p.name_text,
        p.birthdate_local,
        FLOOR(DATEDIFF(day, p.birthdate_local, CURRENT_DATE) / 365.25) AS patient_age_current
...
        pie.identifier_value AS npi
    FROM {source_tables["encntr_participant_ext_table"]} epe
    INNER JOIN {source_tables["practitioner_table"]} p 
        ON epe.participant_indiv_ref = p.practitioner_id 
        AND p.app_source_id = '200' 
        AND p.active_ind = 1
    LEFT JOIN {source_tables["practitioner_identifier_ext_table"]} pie
...
        pie.identifier_value AS pcp_npi
    FROM {source_tables["practitionerrole_table"]} pr
    INNER JOIN {source_tables["practitioner_table"]} p
...
hypertension_dx_problem AS (
    ...
    FROM {source_tables["encounter_table"]} e
    JOIN {source_tables["condition_problem_ext_table"]} cp
...
        cd.patient_reference AS person_id,
        cd.onsetdatetime AS diag_dt_tm
    FROM  {source_tables["condition_problem_ext_table"]} cd
    JOIN {source_tables["nomenclature_ext_table"]} n
        ON n.nomenclature_id = cd.diagnosis_code
...
    LEFT JOIN  {source_tables["nomenclature_ext_table"]} n
        ON n.nomenclature_id = cp.problem_code
...
    FROM encounter e
    JOIN {source_tables["person_table"]} p
...
        AND p.name_text NOT LIKE 'test bob'
        AND p.active_ind = 1
    JOIN {source_tables["observation_table"]} o1
...
        AND o1.valid_until_dt_tm > CURRENT_TIMESTAMP
    JOIN {source_tables["observation_table"]} o2
...
        AND o1.event_tag NOT IN ('in error', 'In Error', 'Date\\Time Correction', 'Cancelled', 'See Note')
        AND o1.valid_until_dt_tm > CURRENT_TIMESTAMP
    LEFT JOIN {source_tables["observation_table"]} o2
        ON e.encounter_id = o2.encounter_reference
        AND o1.effectiveperiod_end = o2.effectiveperiod_end
        AND e.patient_reference = o2.patient_reference
        AND o2.observation_code IN  (3059689, 3788728913)
...
        cs.title AS plan_type_display
    FROM {source_tables["coverage_table"]}c
    JOIN {source_tables["insuranceplan_table"]} ip
        ON ip.insuranceplan_id = c.health_plan_id
        AND ip.active_ind = 1
    LEFT JOIN {source_tables["codesystem_table"]} cs
...
        ON hp.encounter_id = ap.encounter_id
    INNER JOIN {source_tables["regions_providers_table"]} pml1 
...
        ON ep.patient_reference = pcp.person_id
    LEFT JOIN {source_tables["regions_providers_table"]} pml  
