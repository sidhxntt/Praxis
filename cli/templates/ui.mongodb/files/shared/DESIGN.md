# SproutDB — Mongodb-Inspired Landing Page Design

Source specification: `templates/designs/DESIGN-mongodb.md`.

This is an original Praxis Flow implementation inspired by the visual principles documented below. It is not affiliated with or endorsed by the referenced company. The fictional SproutDB brand, all page copy, and the local composition artwork are original. No logos, proprietary product copy, or third-party imagery are included.

## Generated-page intent

Data that grows with the idea. A flexible developer data platform built to make the first prototype and the millionth request feel natural.
