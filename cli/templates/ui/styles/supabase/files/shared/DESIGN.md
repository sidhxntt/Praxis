# Openbase — Supabase-Inspired Landing Page Design

Source specification: `templates/designs/DESIGN-supabase.md`.

This is an original Praxis Flow implementation inspired by the visual principles documented below. It is not affiliated with or endorsed by the referenced company. The fictional Openbase brand, all page copy, and the local composition artwork are original. No logos, proprietary product copy, or third-party imagery are included.

## Generated-page intent

Build openly. Scale confidently. A developer platform pairing a powerful database with the tools every modern product needs.
