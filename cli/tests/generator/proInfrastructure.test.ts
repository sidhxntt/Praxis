...
describe("Praxis Pro infrastructure release contracts", () => {
  it("documents validation, state safety, identity, rollback, and cost controls", async () => {
    const terraform = await readFile(path.resolve("templates/pro/infrastructure/terraform/shared/files/common/infra/terraform/README.md"), "utf8");
    ...
    const kubernetes = await readFile(path.resolve("templates/pro/infrastructure/kubernetes/files/common/k8s/README.md"), "utf8");
