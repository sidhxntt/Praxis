...
The contents of `cli/templates/ui/styles/<style>/` and `cli/templates/ui/catalog/` are generated from design sources and scripts. Edit the canonical sources and renderer under `cli/scripts/ui/`; then run `npm --workspace cli run ui:generate` and `ui:previews`. `ui:verify` proves committed artifacts are current.
