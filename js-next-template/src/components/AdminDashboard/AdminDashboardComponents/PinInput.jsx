"use client";
import React, { useState, useEffect, useMemo, useRef, useCallback, createContext, Children, isValidElement, cloneElement } from "react";
...
const usePinInput = ({ value, defaultValue, placeholder, type, length, readOnly }) => {
  ...
  const handleKeyDown = (event, index) => {
    ...
    if (type === "numeric" && !/^\d$|Backspace|Tab|Delete|ArrowLeft|ArrowRight| /.test(key) && !(ctrlKey && key === "v") && !(metaKey && key === "v")) {
      event.preventDefault();
    }
...
const getValidChildren = (children) => Children.toArray(children).filter(isValidElement);

const getInputFieldCount = (children) => Children.toArray(children).filter((child) => isValidElement(child) && child.type === PinInputField).length;
