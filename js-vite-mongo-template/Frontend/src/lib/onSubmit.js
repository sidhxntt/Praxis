...
export const onUserSubmit = async (data, userId, form, onOpenChange) => {
  try {
...
export const onTaskSubmit = async (data, taskId, form, onOpenChange) => {
  try {
    const token = Cookies.get("access_token");
    if (!token) {
      throw new Error("No authentication token found.");
...
export const onUserInviteSubmit = async (data, form, onOpenChange) => {
  try {
    const token = Cookies.get("access_token");
    if (!token) {
      throw new Error("No authentication token found.");
